	...
	while(co != 1251)
	{
		...
		for(i=1;i<=count-1;i++)// ↓ 
		{
			for(j=(i+1);j<=count;j++)//-->
			{
				...
				pheromone[i][j] = (antPheromone/distance[i][j]) * pathHaveAntNoDirection[i][j] + (1-p) * pheromone[i][j];
			}
		}		
		for(i=0;i<ant;i++)		
		{
...
//	write call gnuplot 	
	// "->要打成 \"  
	FILE *gp = popen(" \"C:\\Users\\YiLin\\Downloads\\gp524-win64-mingw_2\\gnuplot\\bin\\gnuplot.exe\" -persist","w");					
	fprintf(gp, "load \'C:\\Users\\YiLin\\OneDrive\\File\\Summer_Vacation_Algo\\HW5\\PlotPoint\\plot.plt\'\n");
	fflush(gp);
	endclock = clock();		
